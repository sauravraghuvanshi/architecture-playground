...
const Inner = forwardRef<BaseCanvasHandle, Props>(function Inner({ value, onChange }, ref) {
  ...
  const { onNodesChange, onEdgesChange, setNodes, setEdges, snapshot, nodes, edges } = useFlowCanvas<UMLPayload>({
  ...
  // BuilderPalette: add a fresh class / interface / abstract at viewport center.
  useEffect(() => {
    const handler = (e: Event) => {
      const ce = e as CustomEvent<{ kind?: string }>;
      const kind = ce.detail?.kind ?? "class";
      const center = screenToFlowPosition({ x: window.innerWidth / 2, y: window.innerHeight / 2 });
      const id = `C_${Date.now().toString(36)}`;
      const stereotype = kind === "interface" ? "interface" : kind === "abstract" ? "abstract" : undefined;
      const name = kind === "interface" ? "INewInterface" : kind === "abstract" ? "AbstractBase" : "NewClass";
      snapshot();
      setNodes((nds) => nds.concat({
        id, type: "uml",
        position: { x: center.x - 120, y: center.y - 60 },
        data: { name, stereotype, fields: [], methods: [] },
      }));
    };
    window.addEventListener("uml-add-class", handler as EventListener);
    return () => window.removeEventListener("uml-add-class", handler as EventListener);
